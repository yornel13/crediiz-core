...
A CRM system designed for client acquisition campaigns in Panama. The platform enables an **Owner/Administrator** to upload client lists, assign them to field **Agents**, and track call activity and outcomes in real time. Agents use a mobile app to call assigned clients sequentially, write open notes during calls, mark the outcome, schedule follow-ups for interested clients, and sync data back to the CRM — even under unreliable network conditions.
...
- Creates and manages Agent accounts (CRUD).
- Assigns clients to Agents.
- Views call activity dashboard (call counts, outcomes, notes — read-only).
- Views the **Agent agendas**: interested clients, scheduled follow-ups, and their dates — filterable by Agent. Read-only supervision.
- Can reassign clients to a different Agent if needed.
- Can manually change client status (e.g., CONVERTED, DO_NOT_CALL).

### 3.2 Agent (Mobile App)
...
| **Client Assignment** | Assign one or more clients to an Agent |
| **Agent CRUD** | Create, edit, activate/deactivate Agent accounts |
| **Follow-Up Supervision** | View all scheduled follow-ups across all Agents or filtered by Agent. See client name, scheduled date/time, reason, and status (pending/completed). Read-only |
| **Interested Clients** | View all clients marked as INTERESTED — filterable by Agent |
| **Dashboard** | Total calls per Agent, outcome counts (answered, not answered), avg call duration, unique clients contacted, follow-up stats, client notes (read-only). Supports `?from=` and `?to=` query params for date filtering (daily/weekly/monthly reports). Numbers and text only — no complex charts |

### 4.2 Mobile App (Android)

| Feature | Description |
|---|---|
| **Auth** | Agent login with JWT |
...
| **Open Note** | A free-text field that stays open during the entire call. The Agent writes observations in real time. Opens via tap on "Notes" button in the in-call screen |
| **Outcome Selection** | After call ends, Agent selects one outcome: Interested, Not Interested, No Answer, Busy, Invalid Number |
| **Follow-Up Scheduling** | When outcome is Interested: date picker, time picker, and reason field appear. The Agent schedules when to call back |
| **Follow-Up Notifications** | Local notification at the scheduled time as a reminder |
| **Offline Mode** | Local persistence (Room DB) of clients, interactions, notes, and follow-ups |
| **Sync on Call End** | Attempts immediate sync of interaction + notes + follow-up after each call |
...
|---|---|---|---|
| **Interested** | → `INTERESTED` | Agent schedules follow-up (date + time + reason) | Client shows interest. Goes to agenda |
...
PENDING ──────────────────────────────────────────────────────
  ...
  │                              │                            
  │                    Agent calls back on scheduled date      
  │                              │                            
...
## 6. Follow-Up System (Agent Agenda)

### 6.1 How It Works

1. Agent calls a client → marks outcome as **Interested**.
...
4. The Agent's **Agenda tab** shows all scheduled follow-ups grouped by date.
5. On the scheduled date, a **local notification** reminds the Agent.
6. The Agent calls from the Agenda (tap or auto-call mode).
7. After the follow-up call, the Agent can:
...
```
PENDING ──► (scheduled date arrives) ──► Agent calls ──► COMPLETED
   │                                                           │
   │  Agent or Owner cancels                     New FollowUp if rescheduled
   ▼
CANCELLED
```
...
- **All follow-ups** across all Agents, or filtered by Agent.
- Each entry shows: client name, Agent name, scheduled date/time, reason, status.
- The Owner does NOT create or edit follow-ups — this is the Agent's tool.
- The Owner CAN reassign a client to a different Agent (existing follow-ups for the old Agent are cancelled, the new Agent manages their own agenda).

---

## 7. Offline-First Sync Strategy

1. **Local persistence**: The Android app stores assigned clients, interactions, notes, and follow-ups in a local Room database.
2. **Sync on call end**: After each call + note + outcome + follow-up, the app immediately attempts to POST data to the backend.
3. **Deduplication**: Each interaction, note, and follow-up carries a `mobileSyncId` (UUID generated on the device). The backend uses unique indexes to reject duplicates silently.
4. **Background sync**: Android `WorkManager` retries pending syncs every 15–30 minutes or when network connectivity is restored.
5. **Conflict-free**: The device only creates data (interactions, notes, follow-ups), never updates existing server records. This makes sync conflict-free by design.
...
- Agent-to-Agent client transfer (only Owner can reassign).
- Owner creating or editing follow-ups (Agent-only tool, Owner has read-only visibility).
